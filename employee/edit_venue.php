...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') { $error = 'Venue name is required.'; }
        ...
        $stmt = $pdo->prepare('UPDATE venues SET name=:name, address=:address, capacity=:capacity WHERE id=:id');
        $stmt->execute(['name'=>$venue['name'], 'address'=>$venue['address'], 'capacity'=>$venue['capacity'], 'id'=>$id]);
